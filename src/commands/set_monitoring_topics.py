...
async def study_monitoring_topic(update: Update, context: ContextTypes.DEFAULT_TYPE)-> None:  
    """"""  
...
async def weekly_monitoring_topic(update: Update, context: ContextTypes.DEFAULT_TYPE)-> None:    
    group_id = update.effective_chat.id
    user_id = update.effective_user.id
    current_thread_id = update.message.message_thread_id
    if await is_admin(update, context, user_id) :
